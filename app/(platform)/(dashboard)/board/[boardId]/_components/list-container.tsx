...
export const ListContainer = ({ data, boardId }: ListContainerProps) => {
  ...
  const onDragEnd = (result: any) => {
    ...
    // user drops a list in the same position
    if (
    ...
    if (type === "card") {
      ...
      // check if cards exist in source
      if (!sourceList.cards) {
        sourceList.cards = [];
      }

      // check if cards exist in destination
      if (!destList.cards) {
        destList.cards = [];
      }
      ...
      if (source.droppableId === destination.droppableId) {
        ...
        // asign new order to the moved card
      ...
      <Droppable droppableId="lists" type="list" direction="horizontal">
        {provided => (
          ...
          >
            ...
            <React.Fragment key="key-placeholder-1">
              {provided.placeholder}
            </React.Fragment>
            <ListForm key="list-form" />
            <div key="spacer" className="flex shrink-0 w-1" />
          </ol>
        )}
      </Droppable>
    </DragDropContext>
